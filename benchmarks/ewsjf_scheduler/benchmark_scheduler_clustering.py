...
async def main_ewsjf(queues_config):
    ...
    for i in range(len(configs)):
        external_parameters = {"queues_config": configs[i], "step_size": 1500}
        ...
        await run_engine(engine_args, prompts, sampling_params, rates[0])
...
def get_queues_config(data):
    ...
    p = multiprocessing.Process(target=builder.build_queues_multiprocessing, args=(queue, data,))
    p.start()
    queues = queue.get()
    p.terminate()
...
if __name__ == "__main__":
    ...
    asyncio.run(main_ewsjf(queues_30_100_16000))
